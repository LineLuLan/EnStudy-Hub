...
import { Eraser, Lightbulb, Volume2, VolumeX, X } from 'lucide-react';
...
export function ListeningCard({
  ...
  // Track whether the browser supports speechSynthesis. Set on mount so SSR
  // hydration matches (we render the same UI server vs client).
  ...
  const playWord = useCallback(() => {
    ...
    setPlaying(true);
    speakWord(word);
    // Reset the playing indicator after a heuristic duration: roughly 250ms
    // per letter, capped to a couple seconds. Web Speech doesn't fire a
    // reliable 'end' event across browsers, so we use this for UX feedback only.
    const eta = Math.min(2200, Math.max(500, word.length * 250));
    const t = window.setTimeout(() => setPlaying(false), eta);
    return () => window.clearTimeout(t);
  }, [word]);

  // Auto-play once per card on mount + check support. Toast warns if missing.
  useEffect(() => {
    if (!hasSpeechSupport()) {
      setSupported(false);
      toast.warning('Trình duyệt không hỗ trợ phát âm. Xem chính tả ở dưới rồi gõ lại để luyện.');
      return;
    }
    setSupported(true);
    const cleanup = playWord();
    return cleanup;
  ...
  const unlock = useCallback(
    (gave: boolean) => {
      ...
      setGaveUp(gave);
      setPhase('unlocked');
      // Replay on unlock so the user hears the word again alongside the reveal.
      speakWord(word);
    },
    [phase, word]
  );
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (phase === 'typing') {
        // Space replays the audio cue (only in typing phase — unlock uses
        // Space to submit derived grade).
        if (e.key === ' ') {
          e.preventDefault();
          playWord();
          return;
  ...
  }, [phase, input, wordLower, derivedGrade, appendNextChar, submit, revealHint, giveUp, playWord]);
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="text-center">
          <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
            {supported ? 'Nghe phát âm' : 'Không hỗ trợ phát âm'}
          </div>
          ...
          <button
            ...
            className={`mx-auto mt-3 flex h-20 w-20 items-center justify-center rounded-full border transition-all disabled:cursor-not-allowed disabled:opacity-50 ${
              supported
                ? 'border-sky-200 bg-sky-50 text-sky-700 hover:bg-sky-100 dark:border-sky-900/50 dark:bg-sky-950/30 dark:text-sky-300 dark:hover:bg-sky-950/60'
                : 'border-zinc-200 bg-zinc-50 text-zinc-400 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-600'
            }`}
            aria-label={supported ? 'Phát lại từ' : 'Trình duyệt không hỗ trợ phát âm'}
            title="Phát lại (Space)"
          >
            {supported ? (
              <motion.div
          ...
          <div className="mt-2 text-xs text-zinc-500">
            {supported ? (
              <>
                Bấm <kbd className="rounded border px-1 font-mono text-[10px]">Space</kbd> để phát
                lại
              </>
            ) : (
              <span className="text-amber-600 dark:text-amber-400">
                Fallback: từ sẽ hiện ở khung dưới — gõ theo chính tả
              </span>
            )}
          </div>

          {/* Fallback: when speechSynthesis is missing, surface the word as a
              visible cue so the card stays usable instead of being a dead-end. */}
          {!supported && (
            <div className="mt-3 inline-block rounded bg-zinc-100 px-3 py-1.5 font-mono text-sm tracking-wider text-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
              {word}
            </div>
          )}
